...
/**
 * @Date: create in 10:37 2023/2/27
 * @describe:
 */
@Service
public class OrderServiceImpl implements IOrderService {
    ...
    public ResponseVo<OrderVo> create(Integer uid, Integer shippingId) {
        ...
        //下单的商品通过uid查购物车（获取购物车），校验是否存在某商品 库存是否充足
        //查出选中的商品
        List<Cart> cartList = cartService.listForCart(uid).stream().filter(Cart::isProductSelected).collect(Collectors.toList());
        ...
        Set<Integer> IdSet = productIdSet;
        ...
        for (Cart cart : cartList) {
            ...
            if (!ProductStatusEnum.ON_SALE.getCode().equals(product.getStatus())) {
                return ResponseVo.error(ResponseEnum.PRODUCT_OFF_SALE_DELETE, "商品不是在售状态 " + product.getName());
            }
            //库存是否充足
        ...
        //计算总价格，只计算选中的
        //生成订单，入库（Order表/OrderItem表）(因为程序运行可能会出错，如果在入库时出错，如：order表入库了，但是程序出错了，所以需要使用事务管理)
        ...
        //下单之后要减库存，然后删掉购物车选中的商品
        //Redis有事务（打包命令），不能回滚，所以在此更新购物车
        for (Cart cart : cartList) {
    ...
    //构建order对象 为了不让for循环看起来很乱，所以单独写个方法
    ...
    public ResponseVo<PageInfo> list(Integer uid, Integer pageNum, Integer pageSize) {
        ...
        //这里的map要存放集合(List转为Map)，所以是groupingBy
